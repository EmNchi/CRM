...
export default function CRMPage() {
  const { pipeline: pipelineParam } = useParams<{ pipeline: string }>()
  const [selectedLead, setSelectedLead] = useState<KanbanLead | null>(null)
  const { leads, stages, pipelines, loading, error, handleLeadMove } = useKanbanData()

  const activePipelineName =
  useMemo(() => pipelines.find(p => toSlug(String(p)) === pipelineParam)?.toString() ?? pipelineParam, [pipelines, pipelineParam])
